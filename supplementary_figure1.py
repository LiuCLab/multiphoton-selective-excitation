...
Supplemental file to run to create supplementary figure 1.
...
#################################################################
# Make Figure s1: equivalent one-photon, two-photon, and freq mod slice select using optimal B1z
...
# s1a) Make one photon version
B1Z_AMP = 1.84118378*FZ*2*np.pi/g
pulse = slr_pulse(N, TB, FA, name='s1', DT=DT)
bz_pulse = np.zeros(N)
gz_pulse = np.zeros(N)
...
plot_waveform(fig, outer[cols+1:cols+SEQUENCE_PLOT_END], t, np.abs(RF), np.angle(RF), B1z, Gz, zoom_time=[3, 3.1], RF_lim=[0,5], B1z_lim=[-1.5,1.5])
if PRINT_MAX_VALS:
    print('s1a B1xy^2 integral:' + str(np.trapz(np.power(abs(pulse),2), dx=DT)))
...
plot_waveform(fig, outer[2*cols+1:2*cols+SEQUENCE_PLOT_END], t, np.abs(RF), np.angle(RF), B1z, Gz, zoom_time=[3, 3.1], RF_lim=[0,5], B1z_lim=[-1.5,1.5])
if PRINT_MAX_VALS:
    print('s1b B1xy^2 integral:' + str(np.trapz(np.power(abs(pulse),2), dx=DT)))
...
pulse = fm_pulse(N, TB, FA, FZ, B1Z_AMP, phase=g*B1Z_AMP/(2*np.pi*FZ)-np.pi/2, name='s1', DT=DT)
bz_pulse = np.zeros(N)
...
plot_waveform(fig, outer[3*cols+1:3*cols+SEQUENCE_PLOT_END], t, np.abs(RF), np.angle(RF), B1z, Gz, zoom_time=[3, 3.1], RF_lim=[0,5], B1z_lim=[-1.5,1.5])
if PRINT_MAX_VALS:
    print('s1c B1xy^2 integral:' + str(np.trapz(np.power(abs(pulse),2), dx=DT)))
...
plt.savefig("supplementary_figure1.pdf", bbox_inches=Bbox([[3.1, 0.5], [18.6, 7.5]]))