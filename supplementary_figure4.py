...
Supplemental file to run to create supplementary figure 4.
...
#################################################################
# Make Figure s4: equivalent one-photon, two-photon, and freq mod slice select using optimal B1z and 90 degree flip angle
...
# s4a) Make one photon version
FA = 90
B1Z_AMP = 1.84118378*FZ*2*np.pi/g
pulse = slr_pulse(N, TB, FA, ptype='ex', name='s4', DT=DT)
bz_pulse = np.zeros(N)
gz_pulse = np.zeros(N)
...
plot_waveform(fig, outer[cols+1:cols+SEQUENCE_PLOT_END], t, np.abs(RF), np.angle(RF), B1z, Gz, zoom_time=[3, 3.1], RF_lim=[0,15], B1z_lim=[-1.5,1.5])
if PRINT_MAX_VALS:
    print('s4a B1xy^2 integral:' + str(np.trapz(np.power(abs(pulse),2), dx=DT)))
...
plot_sim(fig, outer[cols+SEQUENCE_PLOT_END], x_vals, np.abs(final_m), 1*np.angle(final_m), M_mag_lim=[0,1.1])


# s1b) Make two-photon version
pulse = slr_pulse(N, TB, FA, freq=FZ, phase=g*B1Z_AMP/(2*np.pi*FZ)-np.pi/2, ptype='ex', name='s4', DT=DT) / (j1(g/(2*np.pi*FZ) * B1Z_AMP))
...
plot_waveform(fig, outer[2*cols+1:2*cols+SEQUENCE_PLOT_END], t, np.abs(RF), np.angle(RF), B1z, Gz, zoom_time=[3, 3.1], RF_lim=[0,15], B1z_lim=[-1.5,1.5])
if PRINT_MAX_VALS:
    print('s4b B1xy^2 integral:' + str(np.trapz(np.power(abs(pulse),2), dx=DT)))
...
plot_sim(fig, outer[2*cols+SEQUENCE_PLOT_END], x_vals, np.abs(final_m), 1*np.angle(final_m), M_mag_lim=[0,1.1])



# s1c) Make frequency modulated version
pulse = fm_pulse(N, TB, FA, FZ, B1Z_AMP, phase=g*B1Z_AMP/(2*np.pi*FZ)-np.pi/2, ptype='ex', name='s4', DT=DT)
bz_pulse = np.zeros(N)
...
plot_waveform(fig, outer[3*cols+1:3*cols+SEQUENCE_PLOT_END], t, np.abs(RF), np.angle(RF), B1z, Gz, zoom_time=[3, 3.1], RF_lim=[0,15], B1z_lim=[-1.5,1.5])
if PRINT_MAX_VALS:
    print('s4c B1xy^2 integral:' + str(np.trapz(np.power(abs(pulse),2), dx=DT)))
...
plt.savefig("supplementary_figure4.pdf", bbox_inches=Bbox([[3.1, 0.5], [18.6, 7.5]]))